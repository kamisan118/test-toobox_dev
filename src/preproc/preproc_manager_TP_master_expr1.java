...
class MyIntComparable_TP_master_expr1 implements Comparator<ArrayList>{
	int tarCol;
	
	public MyIntComparable_TP_master_expr1(int tarCol){
...
public class preproc_manager_TP_master_expr1 {
	...
	public static void combi_results_gen(){
		...
		final String[] inputDir = 				
			{"C:/DBoxs/prim/Dropbox/特設-校務大小事/VBM Lab/2013Q1 master thesis/TP_master_finale/data/+-", 
				"C:/DBoxs/prim/Dropbox/特設-校務大小事/VBM Lab/2013Q1 master thesis/TP_master_finale/data/-+"};				
		...
		for(int i = 0; i < fileList.size(); i++){
			...
			if (filepathString[filepathString.length - 1].equals("Correct.txt")){
				...
				for(int j = 0; j < file_content.size(); j++){										
					...
					output_content_row.add(filepathString[filepathString.length - 2]); // emotion							
		...
		for (int i = 0; i < fileList.size(); i++){
			...
			if (filepathString[filepathString.length - 1].equals("Correct.txt")){				
				try {
					...
					numbersTab.put("CorrectNum", String.valueOf(fileLineCount.count(filepath)/2));
				} catch (IOException e) {
					// TODO Auto-generated catch block
					e.printStackTrace();
				}	
				...
				try {
					path_error = filepath;
					numbersTab.put("ErrorNum", String.valueOf(fileLineCount.count(filepath)/2));
				} catch (IOException e) {
					// TODO Auto-generated catch block
					e.printStackTrace();
				}	
			...
			if(numbersTab.containsKey("CorrectNum") && numbersTab.containsKey("ErrorNum")){
				...
				output_content_row.add(String.valueOf(subject_done.get(subjectName))); // subject
				output_content_row.add(mapTable_order.get(filepathString[filepathString.length - 4] + "," + filepathString[filepathString.length - 2])); // order
				output_content_row.add(filepathString[filepathString.length - 2]); // condition
				...
				Collections.sort(totalrows, 
						new MyIntComparable_TP_master_expr1(0));
	...
	public static void KB_features_DL_brute_SelKeyUD(String st_Key, String st_UD, 
			...
			if (CurrentID != LastID){
				...
				if (isLastSeqSuccess){
					if (new_featureTab){	//如果是第一次開feature table -- 就跑這串; 之後就只是append!
						...
						tmpAL.add(String.valueOf(((String)((ArrayList)al_extract_KeyUD.get(i)).get(1)))); // order
						tmpAL.add(String.valueOf(((String)((ArrayList)al_extract_KeyUD.get(i)).get(2)))); // subjectID
						tmpAL.add(String.valueOf(((String)((ArrayList)al_extract_KeyUD.get(i)).get(3)) ));// emotion
		...
		if (isLastSeqSuccess){
			if (new_featureTab){
				...
				tmpAL.add(String.valueOf(((String)((ArrayList)al_extract_KeyUD.get(al_extract_KeyUD.size()-1)).get(2)))); // subjectID
				tmpAL.add(String.valueOf(((String)((ArrayList)al_extract_KeyUD.get(al_extract_KeyUD.size()-1)).get(3)) ));// emotion
				tmpAL.add(String.valueOf(Long.parseLong((String)calTab.get(ed_KeyUD)) - Long.parseLong((String)calTab.get(st_KeyUD))));
				outputAL.add(tmpAL);
			}
			else{
	...
	public static void main(String[] args) {
		 ...
		 * 
		 * output: keyUDcorrect.csv, general_results.csv, matrix_view.csv 
		 * 	
		 */
		combi_results_gen();
