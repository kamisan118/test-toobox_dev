...
public class TP_expr1_expr2_modifyForCTRLgroup {
	...
	public static void TP_expr1_preproc(String[] inputDir) {
		...
		mapTable_order.put("expr1_ctrl,order1", "1");
		mapTable_order.put("expr1_ctrl,order2", "2");
		...
		for(int i = 0; i < inputDir.length; i++){
			...
			try {
				...
				while( e.hasMoreElements() )
				{
					...
					if ((dirName.equals("order1")) || (dirName.equals("order2"))){
		...
		for (int i = 0; i < fileList_Correct.size(); i++){
			...
			outputAL_row_head.add("c"); // condition
		...
		fileWrite.write_2d_arraylist_string2file("TP_expr1_ctrl_preproc.csv", outputAL, true); // no append		
	}
	...
	public static void TP_expr2_preproc(String[] inputDir) {
		...
		//--------參數設定s-----------
		Hashtable mapTable_order = new Hashtable();
		mapTable_order.put("expr2_fBeep_ctrl,order1", "1");
		mapTable_order.put("expr2_fBeep_ctrl,order2", "2");
		mapTable_order.put("expr2_rBeep_ctrl,order1", "1");
		mapTable_order.put("expr2_rBeep_ctrl,order2", "2");
		...
		for (int i = 0; i < fileList_Correct.size(); i++){
			...
			String BeepType = filepathString[filepathString.length - 4].substring(6,7).toUpperCase(); // BeepType
		...
		fileWrite.write_2d_arraylist_string2file("TP_expr2_ctrl_preproc.csv", outputAL, true); // no append		
	}
	...
	public static void main(String[] args) {
		...
		final String[] expr1_inputDir = 
			{"E:/_DataBase/2013Q1 情緒對鍵盤影響驗證實驗-基於面部情緒刺激(30x2)/_rawData/expr1_ctrl"};
				
		//TP_expr1_preproc(expr1_inputDir);
		
		final String[] expr2_inputDir = 
			{"E:/_DataBase/2013Q1 情緒對鍵盤影響驗證實驗-基於面部情緒刺激(30x2)/_rawData/expr2_fBeep_ctrl", 
			"E:/_DataBase/2013Q1 情緒對鍵盤影響驗證實驗-基於面部情緒刺激(30x2)/_rawData/expr2_rBeep_ctrl"};
